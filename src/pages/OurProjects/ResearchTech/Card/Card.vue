...
const path = '/src/assets/images/our-projects/researchtech/';
</script>
...
<template>
  <div v-if="!smallScreen" class="w-4/12 flex flex-col h-auto">
    ...
    <div class="w-full">
      <h2 class="font-poppins font-[600] text-[#243e45] text-2xl">{{ title }}</h2>
      <h4 class="text-left text-body-text-color font-poppins text-[20px] tablet:text-[16px] font-[400]" v-html="text">
      </h4>
    </div>
  </div>
  <div v-if="smallScreen" class="w-full">
    <div class="flex justify-between items-center w-full gap-[15px]">
      ...
      <div class="w-full flex flex-col justify-end">
        <h2 class="text-[20px] "><b>{{ title }}</b></h2>
