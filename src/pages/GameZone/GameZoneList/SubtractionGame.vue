...
export default {
    ...
    methods: {
        ...
        async playAudio(audioNum) {
            let path = `/assets/subtractionSafari/safari${audioNum + 1}.mp3`;
        ...
        playAudioPath(path) {
            return new Promise((resolve) => {
                ...
                this.currentAudios.push(audio); // Track the audio element
                audio.play();
        ...
        async playIntroAudio() {
            return new Promise((resolve) => {
                let audioIntro = new Audio(
                    "/assets/subtractionSafari/subtractionintro.mp3"
                );
        ...
        handleBeforeUnload(event) {
            this.stopAllAudios(); // Stop all audios before unloading
            if (this.recognition) {
                this.stopRecording();
            }
        },
    ...
    beforeUnmount() {
        this.stopAllAudios(); // Stop all audios on component destroy
        if (this.recognition) {
            this.stopRecording();
        }
